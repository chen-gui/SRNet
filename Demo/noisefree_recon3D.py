...
class Doublel_kan(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.kan1 = KAN([in_channels, out_channels], base_activation=nn.Identity, grid_range=[-1.3, 1.3])

        self.kan2 = KAN([in_channels + out_channels, out_channels], base_activation=nn.Identity, grid_range=[-1.3, 1.3])
    def forward(self, x):
        x1 = self.kan1(x)
        x2 = self.kan2(torch.cat([x, x1], dim=2))
        return x2

class efficient_kan_recon(nn.Module):
    def __init__(self, inchan=2, c1=8, outchannel=128):
        super().__init__()

        self.c1 = Doublel_kan(inchan, c1)
        self.c2 = Doublel_kan(c1, c1*4)
        self.c3 = Doublel_kan(c1*4, c1*8)
        self.c4 = Doublel_kan(c1*8, c1*16)
        self.c5 = Doublel_kan(c1*16, outchannel)
    def forward(self, t):
        t0 = self.c1(t)
        t0 = self.c2(t0)
        t0 = self.c3(t0)
...
multidata = np.load('linear_cleanrecon_multidata.npz')
clean = multidata['clean']
subdata = multidata['subdata']
mask = multidata['mask']
lossdata = np.copy(subdata)

obs_snr = 20 * np.log10(np.linalg.norm(clean) / np.linalg.norm(clean - lossdata))
obs_snr = np.round(obs_snr, 2)
print('obs_snr-SNR:', obs_snr)
...
net = efficient_kan_recon(outchannel=subdata.shape[0]).to(device)
...
for epoch in range(num_epoch):
    ...
    if (epoch + 1) % 1000 == 0:
        # torch.save(net, '{}-model.pth'.format(int(epoch + 1)))

        out_data = torch.zeros((1, 1, subdata.shape[0]), dtype=torch.float32)
        with torch.no_grad():
            net.eval()
        ...
        recon_snr = 20 * np.log10(np.linalg.norm(clean) / np.linalg.norm(clean - subdata))
        recon_snr = np.round(recon_snr, 2)
        print('recon_snr-SNR:', recon_snr)
    #
    #
    #     # plt.figure()
    #     # plt.imshow(clean[:, :, 8], cmap=cseis(), vmin=-1, vmax=1, aspect='auto', interpolation="None")
    #     #
    #     # plt.figure()
    #     # plt.imshow(lossdata[:, :, 8], cmap=cseis(), vmin=-1, vmax=1, aspect="auto", interpolation="None")
    #
    #     plt.figure()
    #     plt.imshow(subdata[:, :, 8], cmap=cseis(), vmin=-1, vmax=1, aspect="auto", interpolation="None")
    #
    #     plt.figure()
    #     plt.imshow(clean[:, :, 8]-subdata[:, :, 8], cmap=cseis(), vmin=-1, vmax=1, aspect="auto", interpolation="None")
    #     plt.show()